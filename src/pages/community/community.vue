    ...
    <div style="width: 1200px; margin: 0 auto; display: flex;">
      <div class="cont-main">
        <header class="play-header">
          <span class="play-header-title play-header-title-active" v-link="{name:'community-mine',params:{id:this.degree_id}}">我的问答</span>
          <span class="play-header-title" v-link="{name:'community-all',params:{id:this.degree_id}}" style="margin-left: 30px;">全部问题</span>
        </header>
        <div v-show="!comment_show" style="width: 814px;">
          <input class="play-question-title" v-model="data.title" type="text" placeholder="请输入问题标题" name="questionTitle">

          <div style="padding: 10px;" contenteditable="true" class="play-edit-question contenteditable-input question-input-content" placeholder="描述具体问题，您的导师将会在2天内解答完毕"></div>

          <div class="play-ques-btn-wrap">
            <button class="play-ques-btn" @click="classAsking()">去提问</button>
          </div>
                    ...
                    <li v-for="(list,comment) in answer.answer_comments" id="{{comment.id}}" class="play-ques-item play-reply" style="display: flex;">
                      ...
                      <span class="play-ques-cont" style="color:#666;">
                        ...
                        <div class="reply-input comment-reply" style="width: 100%; margin-top: 23px; border: 1px solid #E8E8E8;">
                          ...
                          <div style="position: absolute; margin-top: -48px;">
                            <div @click="replying_comment(q_index,item,list)" style="cursor: pointer; position: absolute; margin-left: 562px; text-align: center; line-height: 48px; width: 100px; height: 48px; background: #D8D8D8;font-size: 14px;color: #999999;letter-spacing: 0.22px;">回复</div>
                          </div>
                        </div>
                      </div>
                    </li>
...
<script>
  ...
  export default {
    ...
    ready: function(){
      ...
      this.load(Vue.http.options.xy_root+'/api/v1/degree_bbs/?degree_course_id='+this.degree_id+'&page=1&type=my_questions');
